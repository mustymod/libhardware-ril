...
static int responseCallList(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen % sizeof (RIL_Call15 *) != 0) {
        LOGE("invalid response length %d expected multiple of %d\n",
            (int)responselen, (int)sizeof (RIL_Call15 *));
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    startResponse;
    /* number of call info's */
    num = responselen / sizeof(RIL_Call15 *);
    p.writeInt32(num);

    for (int i = 0 ; i < num ; i++) {
        RIL_Call15 *p_cur = ((RIL_Call15 **) response)[i];
        /* each call info */
        ...
        writeStringToParcel(p, ""/*p, p_cur->name*/);
        p.writeInt32(0/*p_cur->namePresentation*/);
        appendPrintBuf("%s[id=%d,%s,toa=%d,",
            printBuf,
            p_cur->index,
            callStateToString(p_cur->state),
            p_cur->toa);
        appendPrintBuf("%s%s,%s,als=%d,%s,",
            printBuf,
...
static int responseSMS(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof (RIL_SMS_Response15) ) {
        LOGE("invalid response length %d expected %d",
                (int)responselen, (int)sizeof (RIL_SMS_Response15));
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    RIL_SMS_Response15 *p_cur = (RIL_SMS_Response15 *) response;

    p.writeInt32(p_cur->messageRef);
    writeStringToParcel(p, p_cur->ackPDU);
    p.writeInt32(0/*p_cur->errorCode*/);
